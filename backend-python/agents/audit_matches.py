...
def _check_struct(rec, issues):
    ...
    if not rec.get('steps'):
        issues.append('无效局：无决策步骤（开局即结束或异常会话，无标注价值）')
...
def _replay(rec, issues):
    ...
    if rec.get('endedBy') == 'surrender':
        # 认输局：对局在中途截断，胜负由投降判定而非自然结算——豁免胜负一致性检查
        if not rec.get('steps'):
            issues.append('无效局：认输时对局尚未产生任何决策（无标注价值）')
        return
    if not rec.get('steps'):
        return  # 结构检查已标记"无效局"，此处不再重复
    if final != rec['winner']:
        if not rec.get('endedBy'):
            # 旧版代码录制的认输局没有 endedBy 标记：回放无自然胜负但记录有胜者 → 疑似认输
            issues.append('疑似旧版认输局（无 endedBy 标记，胜负与回放不符）——请人工确认是否为认输')
        else:
