 ...
 * 并按以下命名规则放置，无需修改任何代码：
 *
 *   character/{id}.png     → 人物角色插画（如 character/601.png）
 *   action/{index}.png     → 行动卡插画（如 action/0.png）
 *   marker/{type}.png      → 标记插画（marker/jiugongge.png / marker/shizi.png）
 *   dead.png               → 死亡角色插画
 */
...
export function markerIllustration(shape: string, round?: number): string {
  ...
  return `${BASE}/marker/r${r}_${key}.png`;
}

/** 监视标记图片路径（round: 所属回合；第 2~6 回合用金色带回合号标记 golden_mark_r{N}.png，其余回退到第 2 回合） */
export function surveillanceIllustration(round?: number): string {
  const r = round != null && round >= 2 && round <= 6 ? round : 2;
  return `${BASE}/marker/golden_mark_r${r}.png`;
}

/** 死亡角色图片路径 */
export const deadIllustration = `${BASE}/dead.png`;
